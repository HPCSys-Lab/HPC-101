...
produz(void *n_times)
{
	...
	for (i=0; i<(int)n_times; i++) {
		...
		// !sem_trywait! immediately returns with error !EAGAIN!.
		do {
			result=sem_trywait(&s_prod);
			if(result) {
				strerror_r(errno,err_msg,MAX);
				printf("Produtor: falhou sem_trywait em s_prod(%d): %s\n",result,err_msg);

				// na pratica, deveria fazer outra coisa aqui, em vez de ficar em "busy wait"
...
consome(void *n_times)
{
	...
	for (i=0; i<(int)n_times; i++) {
		do {
			result=sem_trywait(&s_cons);
			if(result) {
				strerror_r(errno,err_msg,MAX);
				printf("Consumidor: falhou sem_trywait em s_cons(%d): %s\n",result,err_msg);
