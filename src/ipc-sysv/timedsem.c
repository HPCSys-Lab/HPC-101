...
int semtimedop(int semid, struct sembuf *sops, unsigned nsops, struct timespec *time_out);
...
lock(int semid, int num_sem)
{
	...
	do {
		...
		// int semtimedop(int semid, struct sembuf *sops, unsigned nsops, struct timespec *time-out);
		result=semtimedop(semid, &ssbuf, 1, &t_out);
